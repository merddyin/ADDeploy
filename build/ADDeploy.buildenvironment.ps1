...
if ((Get-Variable 'BuildEnv' -ErrorAction:SilentlyContinue) -eq $null) {
    $Script:BuildEnv = New-Object -TypeName PSObject -Property @{
        ...
        ModuleTags = 'Tools,AD,provision,delegation,ESAE' -split ','
        ModuleAuthor = 'Topher Whitfield'
        ModuleDescription = 'Creates OU and base delegation structures'
        ...
        # You should NOT enter this key here but rather manually enter it in the AVADeployAD.buildenvironment.json file with: set-buildenvironment -NugetAPIKey '<key>'
    ...
    ########################################
    $PersistentBuildFile = join-path $PSScriptRoot "ADDeploy.buildenvironment.json"
